...
-- Resolve roster name → auth email + whether a password was claimed.
...
begin
  ...
  select id into uid from auth.users where lower(email) = lower(matched.email);
  ...
  if uid is null then
    ...
    insert into auth.users (
      ...
      uid, 'authenticated', 'authenticated', lower(matched.email), encrypted, now(),
      jsonb_build_object('provider', 'email', 'providers', jsonb_build_array('email')),
      jsonb_build_object('full_name', matched.name, 'name', matched.name),
      now(), now(), '', '', '', ''
    );
    insert into auth.identities (
      ...
      gen_random_uuid(), uid,
      jsonb_build_object('sub', uid::text, 'email', lower(matched.email), 'email_verified', true, 'full_name', matched.name),
...
-- Name + password login helper: returns the roster email (Auth validates the password).
...
begin
  select r.email, r.password_set into matched_email, has_password
  from public.resolve_roster_login(p_name) as r;
